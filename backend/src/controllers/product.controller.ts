...
export const createProduct = async (req: Request, res: Response) => {
    try {
        const { title, description, price, image_url, amazon_link, flipkart_link, myntra_link, shopify_link, category, search_keywords } = req.body;
        const { data, error } = await supabaseAdmin.from('products').insert([{ title, description, price, image_url, amazon_link, flipkart_link, myntra_link, shopify_link, category, search_keywords }]).select().single();
        if (error) throw error;
...
export const updateProduct = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const { title, description, price, image_url, amazon_link, flipkart_link, myntra_link, shopify_link, category, search_keywords } = req.body;
        const { data, error } = await supabaseAdmin.from('products').update({ title, description, price, image_url, amazon_link, flipkart_link, myntra_link, shopify_link, category, search_keywords }).eq('id', id).select().single();
        if (error) throw error;
        res.json(data);
    } catch (err: any) {
        res.status(500).json({ error: err.message });
